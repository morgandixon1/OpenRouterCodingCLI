...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in casualresearch-cli/packages/cli/package.json',
  );
}
...
console.log('packing @google/casualresearch-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'google-casualresearch-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @google/casualresearch-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @google/casualresearch-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'google-casualresearch-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @google/casualresearch-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `google-casualresearch-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `google-casualresearch-cli-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.casualresearch_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
