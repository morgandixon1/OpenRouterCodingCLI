...
// User-level .casualresearch directory in home
const USER_casualresearch_DIR = path.join(os.homedir(), '.casualresearch');
// Project-level .casualresearch directory in the workspace
const WORKSPACE_casualresearch_DIR = path.join(projectRoot, '.casualresearch');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.casualresearch/tmp
export const OTEL_DIR = path.join(USER_casualresearch_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .casualresearch directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_casualresearch_DIR,
  'settings.json',
);
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `casualresearch-cli-releases-${Date.now()}.json`,
  );
  try {
    execSync(
      `curl -sL -H "User-Agent: casualresearch-cli-dev-script" -o "${tmpFile}" "${url}"`,
      { stdio: 'pipe' },
    );
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'casualresearch-cli-telemetry-'),
  );
