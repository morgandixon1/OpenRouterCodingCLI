...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    ...
    if (!this.currentIde || !this.currentIdeDisplayName) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run casualresearch CLI in one of these supported IDEs: ${Object.values(
          DetectedIde,
        )
   ...
   * - Runs `command+shift+p` > "casualresearch CLI: Accept Diff in IDE" to accept
   ...
   * - Runs "casualresearch CLI: Close Diff in IDE"
  ...
  private validateWorkspacePath(): boolean {
    const ideWorkspacePath = process.env['casualresearch_CLI_IDE_WORKSPACE_PATH'];
    ...
    if (getRealPath(ideWorkspacePath) !== getRealPath(process.cwd())) {
      this.setState(
        IDEConnectionStatus.Disconnected,
        `Directory mismatch. casualresearch CLI is running in a different location than the open workspace in ${this.currentIdeDisplayName}. Please run the CLI from the same directory as your project's root folder.`,
        true,
      );
      return false;
    }
  ...
  private getPortFromEnv(): string | undefined {
    const port = process.env['casualresearch_CLI_IDE_SERVER_PORT'];
    if (!port) {
      this.setState(
        IDEConnectionStatus.Disconnected,
        `Failed to connect to IDE companion extension for ${this.currentIdeDisplayName}. Please ensure the extension is running and try refreshing your terminal. To install the extension, run /ide install.`,
