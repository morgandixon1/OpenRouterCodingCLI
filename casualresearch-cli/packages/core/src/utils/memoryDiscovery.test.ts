...
import {
  casualresearch_CONFIG_DIR,
  setcasualresearchMdFilename,
...
describe('loadServerHierarchicalMemory', () => {
  ...
  afterEach(async () => {
    ...
    setcasualresearchMdFilename(DEFAULT_CONTEXT_FILENAME);
  ...
  it('should load only the global context file if present and others are not (default filename)', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, casualresearch_CONFIG_DIR, DEFAULT_CONTEXT_FILENAME),
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    setcasualresearchMdFilename(customFilename);

    const customContextFile = await createTestFile(
      path.join(homedir, casualresearch_CONFIG_DIR, customFilename),
  ...
  it('should load ORIGINAL_casualresearch_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootcasualresearchFile = await createTestFile(
      path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
      'Project root memory',
    );
    const srccasualresearchFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, projectRootcasualresearchFile)} ---\nProject root memory\n--- End of Context from: ${path.relative(cwd, projectRootcasualresearchFile)} ---\n\n--- Context from: ${path.relative(cwd, srccasualresearchFile)} ---\nSrc directory memory\n--- End of Context from: ${path.relative(cwd, srccasualresearchFile)} ---`,
      fileCount: 2,
    });
  });

  it('should load ORIGINAL_casualresearch_MD_FILENAME files by downward traversal from CWD', async () => {
    await createTestFile(
  ...
  it('should load and correctly order global, upward, and downward ORIGINAL_casualresearch_MD_FILENAME files', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, casualresearch_CONFIG_DIR, DEFAULT_CONTEXT_FILENAME),
      'default context content',
    );
    const rootcasualresearchFile = await createTestFile(
    ...
    const cwdcasualresearchFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'CWD memory',
    );
    const subDircasualresearchFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, defaultContextFile)} ---\ndefault context content\n--- End of Context from: ${path.relative(cwd, defaultContextFile)} ---\n\n--- Context from: ${path.relative(cwd, rootcasualresearchFile)} ---\nProject parent memory\n--- End of Context from: ${path.relative(cwd, rootcasualresearchFile)} ---\n\n--- Context from: ${path.relative(cwd, projectRootcasualresearchFile)} ---\nProject root memory\n--- End of Context from: ${path.relative(cwd, projectRootcasualresearchFile)} ---\n\n--- Context from: ${path.relative(cwd, cwdcasualresearchFile)} ---\nCWD memory\n--- End of Context from: ${path.relative(cwd, cwdcasualresearchFile)} ---\n\n--- Context from: ${path.relative(cwd, subDircasualresearchFile)} ---\nSubdir memory\n--- End of Context from: ${path.relative(cwd, subDircasualresearchFile)} ---`,
      fileCount: 5,
    });
  });
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const regularSubDircasualresearchFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, regularSubDircasualresearchFile)} ---\nMy code memory\n--- End of Context from: ${path.relative(cwd, regularSubDircasualresearchFile)} ---`,
      fileCount: 1,
    });
  });
  ...
  it('should load extension context file paths', async () => {
    const extensionFilePath = await createTestFile(
      path.join(testRootDir, 'extensions/ext1/casualresearch.md'),
