 ...
 *
 * @param result The result of the tool execution.
 * @param casualresearchClient The casualresearch client to use for summarization.
 * @param abortSignal The abort signal to use for summarization.
 * @returns The summary of the result.
 */
export const defaultSummarizer: Summarizer = (
  result: ToolResult,
  _casualresearchClient: casualresearchClient,
...
export const llmSummarizer: Summarizer = (result, casualresearchClient, abortSignal) =>
  summarizeToolOutput(
...
export async function summarizeToolOutput(
  ...
  try {
    const parsedResponse = (await casualresearchClient.generateContent(
      contents,
      toolOutputSummarizerConfig,
      abortSignal,
      DEFAULT_casualresearch_FLASH_LITE_MODEL,
