...
`;
    ...
    it('should handle custom patterns file name', async () => {
      ...
      await createTestFile('.casualresearchignore', 'temp/\n*.tmp');

      parser.loadPatterns('.casualresearchignore');
      expect(parser.getPatterns()).toEqual(['temp/', '*.tmp']);
      expect(parser.isIgnored(path.join('temp', 'file.txt'))).toBe(true);
      expect(parser.isIgnored(path.join('src', 'file.tmp'))).toBe(true);
    });

    it('should initialize without errors when no .casualresearchignore exists', () => {
      expect(() => parser.loadPatterns('.casualresearchignore')).not.toThrow();
    });
  });
