...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      await fsPromises.writeFile(
        ...
        'ignored.txt\nnode_modules/\n.casualresearch/*\n!/.casualresearch/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
      await createTestFile('ignored.txt');
      await createTestFile('.casualresearch', 'config.yaml');
      await createTestFile('.casualresearch', 'logs.json');
  ...
  describe('with casualresearchignore', () => {
    it('should ignore casualresearchignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.casualresearchignore'),
        'ignored.txt\nnode_modules/\n.casualresearch/\n!/.casualresearch/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectcasualresearchIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.casualresearchignore'),
        'ignored.txt\nnode_modules/\n.casualresearch/\n!/.casualresearch/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectcasualresearchIgnore: false,
          respectGitIgnore: true, // Explicitly disable casualresearch ignore only
        },
      });
      expect(structure).toContain('ignored.txt');
