...
import { createTmpDir, cleanupTmpDir } from '@google/casualresearch-cli-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .casualresearchignore rules', async () => {
    tmpDir = await createTmpDir({
      '.casualresearchignore': 'dist/',
      dist: ['ignored.js'],
      src: ['not-ignored.js'],
    });
    ...
    expect(results).toEqual(['src/', '.casualresearchignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .casualresearchignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.casualresearchignore': 'build/',
      dist: ['ignored-by-git.js'],
      build: ['ignored-by-casualresearch.js'],
      src: ['not-ignored.js'],
    });
