...
export type FileSearchOptions = {
  ...
  usecasualresearchignore: boolean;
  cache: boolean;
  cacheTtl: number;
  maxDepth?: number;
};
 ...
 * respecting .gitignore and .casualresearchignore rules, and utilizing caching
...
export class FileSearch {
  ...
  /**
   * Loads ignore rules from .gitignore and .casualresearchignore files, and applies
  ...
  private loadIgnoreRules(): void {
    ...
    if (this.options.usecasualresearchignore) {
      const casualresearchignorePath = path.join(this.absoluteDir, '.casualresearchignore');
      if (fs.existsSync(casualresearchignorePath)) {
        this.ignore.add(fs.readFileSync(casualresearchignorePath, 'utf8'));
      }
    }
