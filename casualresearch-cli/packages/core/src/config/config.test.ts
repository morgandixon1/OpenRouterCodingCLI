...
import { setcasualresearchMdFilename as mockSetcasualresearchMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../tools/memoryTool', () => ({
  MemoryTool: vi.fn(),
  setcasualresearchMdFilename: vi.fn(),
  getCurrentcasualresearchMdFilename: vi.fn(() => 'casualresearch.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'casualresearch.md',
  casualresearch_CONFIG_DIR: '.casualresearch',
}));
...
vi.mock('../core/client.js', () => ({
  casualresearchClient: vi.fn().mockImplementation(() => ({
...
describe('Server Config (config.ts)', () => {
  const MODEL = 'casualresearch-pro';
  const SANDBOX: SandboxConfig = {
    command: 'docker',
    image: 'casualresearch-cli-sandbox',
  };
  ...
  const EMBEDDING_MODEL = 'casualresearch-embedding';
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      ...
      const authType = AuthType.USE_casualresearch;
      const newModel = 'casualresearch-flash';
      ...
      expect(casualresearchClient).toHaveBeenCalledWith(config);
    ...
    it('should preserve conversation history when refreshing auth', async () => {
      ...
      (
        config as unknown as { casualresearchClient: typeof mockExistingClient }
      ).casualresearchClient = mockExistingClient;
      (casualresearchClient as Mock).mockImplementation(() => mockNewClient);
    ...
    it('should handle case when no existing client is initialized', async () => {
      ...
      // No existing client
      (config as unknown as { casualresearchClient: null }).casualresearchClient = null;
      (casualresearchClient as Mock).mockImplementation(() => mockNewClient);
  ...
  it('Config constructor should call setcasualresearchMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetcasualresearchMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setcasualresearchMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetcasualresearchMdFilename).not.toHaveBeenCalled();
  });
