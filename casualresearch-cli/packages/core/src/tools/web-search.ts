...
/**
 * A tool to perform web searches using Google Search via the casualresearch API.
 */
...
> {
  ...
  constructor(private readonly config: Config) {
    super(
      WebSearchTool.Name,
      'GoogleSearch',
      'Performs a web search using Google Search (via the casualresearch API) and returns the results. This tool is useful for finding information on the internet based on a query.',
      Icon.Globe,
      {
        type: Type.OBJECT,
        properties: {
  ...
  async execute(
    ...
    const casualresearchClient = this.config.getcasualresearchClient();

    try {
      const response = await casualresearchClient.generateContent(
