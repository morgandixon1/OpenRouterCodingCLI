...
describe('LSTool', () => {
  ...
  beforeEach(() => {
    ...
    mockFileService = {
      ...
      shouldcasualresearchIgnoreFile: vi.fn().mockReturnValue(false),
  ...
  describe('execute', () => {
    ...
    it('should respect casualresearchignore patterns', async () => {
      const testPath = '/home/user/project/src';
      ...
      vi.mocked(fs.readdirSync).mockReturnValue(mockFiles as any);
      (mockFileService.shouldcasualresearchIgnoreFile as any).mockImplementation(
      ...
      expect(result.returnDisplay).toBe('Listed 2 item(s). (1 casualresearch-ignored)');
    });
