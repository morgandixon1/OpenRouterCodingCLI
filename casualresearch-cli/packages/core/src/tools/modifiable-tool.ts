...
function createTempFilesForModify(
  ...
  const diffDir = path.join(tempDir, 'casualresearch-cli-tool-modify-diffs');
  ...
  const tempOldPath = path.join(
    diffDir,
    `casualresearch-cli-modify-${fileName}-old-${timestamp}${ext}`,
  );
  const tempNewPath = path.join(
    diffDir,
    `casualresearch-cli-modify-${fileName}-new-${timestamp}${ext}`,
  );
