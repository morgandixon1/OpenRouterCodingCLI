...
/**
 * Sanitizes a schema object in-place to ensure compatibility with the casualresearch API.
 *
...
function _sanitizeParameters(schema: Schema | undefined, visited: Set<Schema>) {
  ...
  // Handle enum values - casualresearch API only allows enum for STRING type
  if (schema.enum && Array.isArray(schema.enum)) {
    if (schema.type !== Type.STRING) {
      // If enum is present but type is not STRING, convert type to STRING
      schema.type = Type.STRING;
    }
    // Filter out null and undefined values, then convert remaining values to strings for casualresearch API compatibility
