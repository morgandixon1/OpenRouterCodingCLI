...
describe('EditTool', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      getcasualresearchClient: vi.fn().mockReturnValue(casualresearchClient),
      ...
      // getcasualresearchConfig: () => ({ apiKey: 'test-api-key' }), // This was not a real Config method
    ...
    it('should use corrected params from ensureCorrectEdit for diff generation', async () => {
      ...
      mockEnsureCorrectEdit.mockImplementationOnce(
        async (_, content, p, client) => {
          ...
          expect(client).toBe(casualresearchClient);
          return {
            params: {
              file_path: filePath,
