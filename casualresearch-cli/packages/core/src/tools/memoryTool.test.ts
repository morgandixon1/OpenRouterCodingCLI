...
const MEMORY_SECTION_HEADER = '## casualresearch Added Memories';
...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset casualresearch_MD_FILENAME to its original value after each test
    setcasualresearchMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setcasualresearchMdFilename', () => {
    it('should update currentcasualresearchMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setcasualresearchMdFilename(newName);
      expect(getCurrentcasualresearchMdFilename()).toBe(newName);
    });

    it('should not update currentcasualresearchMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentcasualresearchMdFilename(); // Get current before trying to change
      setcasualresearchMdFilename('  ');
      expect(getCurrentcasualresearchMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setcasualresearchMdFilename(newNames);
      expect(getCurrentcasualresearchMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllcasualresearchMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('execute (instance method)', () => {
    ...
    it('should call performAddMemoryEntry with correct parameters and return success', async () => {
      ...
      // Use getCurrentcasualresearchMdFilename for the default expectation before any setcasualresearchMdFilename calls in a test
      const expectedFilePath = path.join(
        os.homedir(),
        '.casualresearch',
        getCurrentcasualresearchMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.casualresearch', 'casualresearch.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(path.join('mock', 'home', '.casualresearch'));
        expect(result.fileName).toContain('casualresearch.md');
        expect(result.fileDiff).toContain('Index: casualresearch.md');
        expect(result.fileDiff).toContain('+## casualresearch Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## casualresearch Added Memories');
