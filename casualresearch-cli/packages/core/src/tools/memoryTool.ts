...
export const casualresearch_CONFIG_DIR = '.casualresearch';
export const DEFAULT_CONTEXT_FILENAME = 'casualresearch.md';
export const MEMORY_SECTION_HEADER = '## casualresearch Added Memories';

// This variable will hold the currently configured filename for casualresearch.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setcasualresearchMdFilename.
let currentcasualresearchMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setcasualresearchMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentcasualresearchMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentcasualresearchMdFilename = newFilename.trim();
  }
}

export function getCurrentcasualresearchMdFilename(): string {
  if (Array.isArray(currentcasualresearchMdFilename)) {
    return currentcasualresearchMdFilename[0];
  }
  return currentcasualresearchMdFilename;
}

export function getAllcasualresearchMdFilenames(): string[] {
  if (Array.isArray(currentcasualresearchMdFilename)) {
    return currentcasualresearchMdFilename;
  }
  return [currentcasualresearchMdFilename];
...
function getGlobalMemoryFilePath(): string {
  return path.join(homedir(), casualresearch_CONFIG_DIR, getCurrentcasualresearchMdFilename());
}
