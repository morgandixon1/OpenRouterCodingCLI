...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .casualresearchignore patterns even when not in a git repo', async () => {
      await createTestFile('.casualresearchignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldcasualresearchIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldcasualresearchIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.casualresearchignore', 'logs/');
    });

    it('should filter out git-ignored and casualresearch-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectcasualresearchIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectcasualresearchIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldcasualresearchIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.casualresearchignore', '*.log');
    });
    ...
    it('should return true for casualresearch-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldcasualresearchIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-casualresearch-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldcasualresearchIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
