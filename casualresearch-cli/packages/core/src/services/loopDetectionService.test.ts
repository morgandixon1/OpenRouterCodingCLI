...
describe('LoopDetectionService', () => {
  ...
  const createContentEvent = (content: string): ServercasualresearchContentEvent => ({
  ...
  describe('Tool Call Loop Detection', () => {
    ...
    it('should not reset tool call counter for other event types', () => {
      ...
      const otherEvent = {
        type: 'thought',
      } as unknown as ServercasualresearchStreamEvent;
...
describe('LoopDetectionService LLM Checks', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      getcasualresearchClient: () => mockcasualresearchClient,
      getDebugMode: () => false,
      getTelemetryEnabled: () => true,
    } as unknown as Config;
  ...
  it('should trigger LLM check on the 30th turn', async () => {
    mockcasualresearchClient.generateJson = vi
      .fn()
  ...
  it('should adjust the check interval based on confidence', async () => {
    ...
    expect(mockcasualresearchClient.generateJson).toHaveBeenCalledTimes(2);
  });
