...
import { casualresearchEventType, ServercasualresearchStreamEvent } from '../core/turn.js';
import { logLoopDetected } from '../telemetry/loggers.js';
import { LoopDetectedEvent, LoopType } from '../telemetry/types.js';
import { Config, DEFAULT_casualresearch_FLASH_MODEL } from '../config/config.js';
...
export class LoopDetectionService {
   ...
   */
  addAndCheck(event: ServercasualresearchStreamEvent): boolean {
    if (this.loopDetected) {
      return true;
    }

    switch (event.type) {
      case casualresearchEventType.ToolCallRequest:
        // content chanting only happens in one single stream, reset if there
        // is a tool call in between
        this.resetContentTracking();
        this.loopDetected = this.checkToolCallLoop(event.value);
        break;
      case casualresearchEventType.Content:
...
Please analyze the conversation history to determine the possibility that the conversation is stuck in a repetitive, non-productive state.`;
    ...
    try {
      result = await this.config
        .getcasualresearchClient()
        .generateJson(contents, schema, signal, DEFAULT_casualresearch_FLASH_MODEL);
    } catch (e) {
