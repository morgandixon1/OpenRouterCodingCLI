...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as casualresearchChat, 'prompt-id-1');
  ...
  describe('run', () => {
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServercasualresearchToolCallRequestEvent;
      expect(event1.type).toBe(casualresearchEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServercasualresearchToolCallRequestEvent;
      expect(event2.type).toBe(casualresearchEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: casualresearchEventType.Content, value: 'First part' },
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServercasualresearchErrorEvent;
      expect(errorEvent.type).toBe(casualresearchEventType.Error);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServercasualresearchToolCallRequestEvent;
      expect(event3.type).toBe(casualresearchEventType.ToolCallRequest);
    ...
    it('should yield finished event when response has finish reason', async () => {
      ...
      expect(events).toEqual([
        { type: casualresearchEventType.Content, value: 'Partial response' },
        { type: casualresearchEventType.Finished, value: 'STOP' },
      ]);
    });
    ...
    it('should yield finished event for MAX_TOKENS finish reason', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: casualresearchEventType.Finished, value: 'MAX_TOKENS' },
      ]);
    });
    ...
    it('should yield finished event for SAFETY finish reason', async () => {
      ...
      expect(events).toEqual([
        { type: casualresearchEventType.Content, value: 'Content blocked' },
        { type: casualresearchEventType.Finished, value: 'SAFETY' },
      ]);
    });
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: casualresearchEventType.Content, value: 'First part' },
        { type: casualresearchEventType.Content, value: 'Second part' },
        { type: casualresearchEventType.Finished, value: 'OTHER' },
      ]);
    });
  });
