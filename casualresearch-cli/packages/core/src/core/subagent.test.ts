...
async function createMockConfig(
  ...
  const configParams: ConfigParameters = {
    ...
    model: DEFAULT_casualresearch_MODEL,
    targetDir: '.',
    debugMode: false,
    cwd: process.cwd(),
  };
...
describe('subagent.ts', () => {
  ...
  describe('SubAgentScope', () => {
    ...
    const defaultModelConfig: ModelConfig = {
      model: 'casualresearch-1.5-flash-latest',
    ...
    beforeEach(async () => {
      ...
      vi.mocked(casualresearchChat).mockImplementation(
        () =>
          ({
    ...
    const getGenerationConfigFromMock = (
      ...
      const callArgs = vi.mocked(casualresearchChat).mock.calls[callIndex];
    ...
    describe('runNonInteractive - Initialization and Prompting', () => {
      it('should correctly template the system prompt and initialize casualresearchChat', async () => {
        const { config } = await createMockConfig();

        vi.mocked(casualresearchChat).mockClear();
        ...
        // Check if casualresearchChat was initialized correctly by the subagent
        expect(casualresearchChat).toHaveBeenCalledTimes(1);
        const callArgs = vi.mocked(casualresearchChat).mock.calls[0];
