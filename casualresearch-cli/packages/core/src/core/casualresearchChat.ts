 ...
 */
export class casualresearchChat {
  ...
  getSystemInstruction(): string | undefined {
    if (typeof this.generationConfig.systemInstruction === 'string') {
      return this.generationConfig.systemInstruction;
    }
    if (typeof this.generationConfig.systemInstruction === 'object' && 
        this.generationConfig.systemInstruction && 
        'parts' in this.generationConfig.systemInstruction &&
        this.generationConfig.systemInstruction.parts) {
      return this.generationConfig.systemInstruction.parts.map((part: any) => part.text).join('\n');
    }
    return undefined;
  }
  /**
   ...
   * @example
   * ```ts
   * const chat = ai.chats.create({model: 'casualresearch-2.0-flash'});
  ...
  async sendMessage(
    ...
    try {
      const apiCall = () => {
        const modelToUse = this.config.getModel() || DEFAULT_casualresearch_FLASH_MODEL;

        // Prevent Flash model calls immediately after quota error
        if (
          this.config.getQuotaErrorOccurred() &&
          modelToUse === DEFAULT_casualresearch_FLASH_MODEL
        ) {
          throw new Error(
