...
import { findIndexAfterFraction, casualresearchClient } from './client.js';
import { AuthType, ContentGenerator } from './contentGenerator.js';
import { casualresearchChat } from './casualresearchChat.js';
import { Config } from '../config/config.js';
import { casualresearchEventType, Turn } from './turn.js';
...
describe('casualresearch Client (client.ts)', () => {
  let client: casualresearchClient;
  beforeEach(async () => {
    vi.resetAllMocks();
    ...
    // Because the casualresearchClient constructor kicks off an async process (startChat)
    ...
    client = new casualresearchClient(
      new Config({ sessionId: 'test-session-id' } as never),
    );
    mockConfigObject.getcasualresearchClient.mockReturnValue(client);
  ...
  // was not being detected as called by the casualresearchClient instance.
  // This likely points to a subtle issue in how the GoogleGenerativeAI class constructor
  // and its instance methods are mocked and then used by the class under test.
  // For future debugging, ensure that the `this.client` in `casualresearchClient` (which is an
  ...
  // For future debugging, ensure `this.client.models.generateContent` in `casualresearchClient` correctly
  ...
  describe('tryCompressChat', () => {
    ...
    beforeEach(() => {
      ...
      } as unknown as casualresearchChat;
    });
  ...
  describe('sendMessageStream', () => {
    it('should include IDE context when ideModeFeature is enabled', async () => {
      ...
      const mockChat: Partial<casualresearchChat> = {
        addHistory: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
      };
      client['chat'] = mockChat as casualresearchChat;
...
- /path/to/recent/file2.ts
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: casualresearchEventType.MaxSessionTurns }]);
  ...
  describe('handleFlashFallback', () => {
    it('should use current model from config when checking for fallback', async () => {
      const initialModel = client['config'].getModel();
      const fallbackModel = DEFAULT_casualresearch_FLASH_MODEL;
