...
function isThinkingSupported(model: string) {
  if (model.startsWith('casualresearch-2.5')) return true;
  return false;
}
...
export class casualresearchClient {
  private chat?: casualresearchChat;
  ...
  async startChat(extraHistory?: Content[]): Promise<casualresearchChat> {
    ...
    try {
      ...
      await reportError(
        error,
        'Error initializing casualresearch chat session.',
        history,
        'startChat',
      );
  ...
  async *sendMessageStream(
  ...
  ): AsyncGenerator<ServercasualresearchStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: casualresearchEventType.MaxSessionTurns };
    ...
    if (compressed) {
      yield { type: casualresearchEventType.ChatCompressed, value: compressed };
    }
    ...
    if (loopDetected) {
      yield { type: casualresearchEventType.LoopDetected };
      return turn;
    }
  ...
  async generateJson(
    ...
    const modelToUse =
      model || this.config.getModel() || DEFAULT_casualresearch_FLASH_MODEL;
    try {
      const userMemory = this.config.getUserMemory();
      const systemInstruction = getCoreSystemPrompt(userMemory);
      const requestConfig = {
