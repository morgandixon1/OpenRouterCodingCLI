...
export class Logger {
  private casualresearchDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.casualresearchDir = getProjectTempDir(process.cwd());
    this.logFilePath = path.join(this.casualresearchDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.casualresearchDir, { recursive: true });
  ...
  _checkpointPath(tag: string): string {
    ...
    return path.join(this.casualresearchDir, `checkpoint-${sanitizedTag}.json`);
  }
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.casualresearchDir) {
      console.error(
