...
const casualresearch_DIR_NAME = '.casualresearch';
...
const TEST_casualresearch_DIR = path.join(
  os.homedir(),
  casualresearch_DIR_NAME,
  TMP_DIR_NAME,
  hash,
);

const TEST_LOG_FILE_PATH = path.join(TEST_casualresearch_DIR, LOG_FILE_NAME);
const TEST_CHECKPOINT_FILE_PATH = path.join(
  TEST_casualresearch_DIR,
  CHECKPOINT_FILE_NAME,
);

async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_casualresearch_DIR, { recursive: true, force: true });
  } catch (_error) {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_casualresearch_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    it('should create .casualresearch directory and an empty log file if none exist', async () => {
      const dirExists = await fs
        .access(TEST_casualresearch_DIR)
        .then(() => true)
        .catch(() => false);
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_casualresearch_DIR);
      expect(
        dirContents.some(
          (f) =>
  ...
  describe('checkpointExists', () => {
    ...
    beforeEach(() => {
      taggedFilePath = path.join(TEST_casualresearch_DIR, `checkpoint-${tag}.json`);
    });
