...
export async function runAcpPeer(config: Config, settings: LoadedSettings) {
  ...
  new acp.ClientConnection(
    (client: acp.Client) => new casualresearchAgent(config, settings, client),
    stdout,
    stdin,
  );
}

class casualresearchAgent implements Agent {
  chat?: casualresearchChat;
  ...
  async sendUserMessage(params: acp.SendUserMessageParams): Promise<void> {
    ...
    if (!this.chat) {
      ...
      this.chat = await casualresearchClient.startChat();
    }
