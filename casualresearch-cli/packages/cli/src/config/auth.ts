...
export const validateAuthMethod = (authMethod: string): string | null => {
  ...
  if (authMethod === AuthType.USE_casualresearch) {
    if (!process.env.casualresearch_API_KEY) {
      return 'casualresearch_API_KEY environment variable not found. Add that to your environment and try again (no reload needed if using .env)!';
    }
    return null;
  }
