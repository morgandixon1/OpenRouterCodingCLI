...
export const SETTINGS_DIRECTORY_NAME = '.casualresearch';
export const USER_SETTINGS_DIR = path.join(homedir(), SETTINGS_DIRECTORY_NAME);
export const USER_SETTINGS_PATH = path.join(USER_SETTINGS_DIR, 'settings.json');
export const DEFAULT_EXCLUDED_ENV_VARS = ['DEBUG', 'DEBUG_MODE'];

export function getSystemSettingsPath(): string {
  if (process.env.casualresearch_CLI_SYSTEM_SETTINGS_PATH) {
    return process.env.casualresearch_CLI_SYSTEM_SETTINGS_PATH;
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/casualresearchCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\casualresearch-cli\\settings.json';
  } else {
    return '/etc/casualresearch-cli/settings.json';
  }
}
...
export interface Settings {
  ...
  /** The model name to use (e.g 'casualresearch-9.0-pro') */
  model?: string;
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // prefer casualresearch-specific .env under casualresearch_DIR
    const casualresearchEnvPath = path.join(currentDir, casualresearch_DIR, '.env');
    if (fs.existsSync(casualresearchEnvPath)) {
      return casualresearchEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      // check .env under home as fallback, again preferring casualresearch-specific .env
      const homecasualresearchEnvPath = path.join(homedir(), casualresearch_DIR, '.env');
      if (fs.existsSync(homecasualresearchEnvPath)) {
        return homecasualresearchEnvPath;
      }
...
export function loadEnvironment(settings?: Settings): void {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(casualresearch_DIR);
