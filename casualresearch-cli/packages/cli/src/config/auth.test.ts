...
describe('validateAuthMethod', () => {
  ...
  describe('USE_casualresearch', () => {
    it('should return null if casualresearch_API_KEY is set', () => {
      process.env.casualresearch_API_KEY = 'test-key';
      expect(validateAuthMethod(AuthType.USE_casualresearch)).toBeNull();
    });

    it('should return an error message if casualresearch_API_KEY is not set', () => {
      expect(validateAuthMethod(AuthType.USE_casualresearch)).toBe(
        'casualresearch_API_KEY environment variable not found. Add that to your environment and try again (no reload needed if using .env)!',
      );
    });
  });
