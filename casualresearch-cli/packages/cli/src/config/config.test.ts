...
import { ShellTool, EditTool, WriteFileTool } from '@google/casualresearch-cli-core';
import { loadCliConfig, parseArguments } from './config.js';
import { Settings } from './settings.js';
import { Extension } from './extension.js';
import * as ServerConfig from '@google/casualresearch-cli-core';
...
vi.mock('@google/casualresearch-cli-core', async () => {
  const actualServer = await vi.importActual<typeof ServerConfig>(
    '@google/casualresearch-cli-core',
  );
  return {
    ...actualServer,
    IdeClient: {
...
describe('loadCliConfig', () => {
  ...
  beforeEach(() => {
    ...
    process.env.casualresearch_API_KEY = 'test-api-key'; // Ensure API key is set for tests
  });
...
describe('loadCliConfig telemetry', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(os.homedir).mockReturnValue('/mock/home/user');
    process.env.casualresearch_API_KEY = 'test-api-key';
  });
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
  ...
  it('should pass extension context file paths to loadServerHierarchicalMemory', async () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/casualresearch.md'],
      },
      {
        config: {
          name: 'ext2',
    ...
    expect(ServerConfig.loadServerHierarchicalMemory).toHaveBeenCalledWith(
      ...
      expect.any(Object),
      [
        '/path/to/ext1/casualresearch.md',
        '/path/to/ext3/context1.md',
        '/path/to/ext3/context2.md',
      ],
      'tree',
  ...
  // To re-enable tests for loadHierarchicalcasualresearchMemory, ensure that:
  ...
  it('should correctly use mocked homedir for global path', async () => {
    const MOCK_casualresearch_DIR_LOCAL = path.join('/mock/home/user', '.casualresearch');
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_casualresearch_DIR_LOCAL, 'casualresearch.md');
    mockFs({
      [MOCK_GLOBAL_PATH_LOCAL]: { type: 'file', content: 'GlobalContentOnly' }
    });
    const memory = await loadHierarchicalcasualresearchMemory("/some/other/cwd", false);
...
describe('loadCliConfig model selection', () => {
  it('selects a model from settings.json if provided', async () => {
    ...
    const config = await loadCliConfig(
      {
        model: 'casualresearch-9001-ultra',
      },
      [],
      'test-session',
      argv,
    ...
    expect(config.getModel()).toBe('casualresearch-9001-ultra');
  });

  it('uses the default casualresearch model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe('casualresearch-2.5-pro');
  });

  it('always prefers model from argvs', async () => {
    process.argv = ['node', 'script.js', '--model', 'casualresearch-8675309-ultra'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
      {
        model: 'casualresearch-9001-ultra',
    ...
    expect(config.getModel()).toBe('casualresearch-8675309-ultra');
  });
...
describe('loadCliConfig ideModeFeature', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.casualresearch_CLI_IDE_SERVER_PORT;
  });
