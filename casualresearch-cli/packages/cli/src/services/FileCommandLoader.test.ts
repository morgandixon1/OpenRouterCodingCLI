  ...
  describe('Extension Command Loading', () => {
    it('loads commands from active extensions', async () => {
      ...
      const extensionDir = path.join(
        process.cwd(),
        '.casualresearch/extensions/test-ext',
      );
      ...
      mock({
        ...
        [extensionDir]: {
          'casualresearch-extension.json': JSON.stringify({
            name: 'test-ext',
            version: '1.0.0',
          }),
          commands: {
    ...
    it('only loads commands from active extensions', async () => {
      const extensionDir1 = path.join(
        process.cwd(),
        '.casualresearch/extensions/active-ext',
      );
      const extensionDir2 = path.join(
        process.cwd(),
        '.casualresearch/extensions/inactive-ext',
      );
    ...
    it('handles missing extension commands directory gracefully', async () => {
      const extensionDir = path.join(
        process.cwd(),
        '.casualresearch/extensions/no-commands',
      );
    ...
    it('handles nested command structure in extensions', async () => {
      const extensionDir = path.join(process.cwd(), '.casualresearch/extensions/a');
