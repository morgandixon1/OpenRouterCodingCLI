...
describe('parseAndFormatApiError', () => {
  const _enterpriseMessage =
    'upgrade to a casualresearch Code Assist Standard or Enterprise plan with higher limits';
  const vertexMessage = 'request a quota increase through Vertex';
  const casualresearchMessage = 'request a quota increase through AI Studio';
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the casualresearch-2.5-flash model',
    );
  });
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "casualresearch 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.google.dev/casualresearch-api/docs/rate-limits.",
    ...
    const result = parseAndFormatApiError(errorMessage, AuthType.USE_casualresearch);
    expect(result).toContain('casualresearch 2.5 Pro Preview');
    expect(result).toContain(casualresearchMessage);
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Free tier)', () => {
    const errorMessage =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'casualresearch 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    const result = parseAndFormatApiError(
      errorMessage,
      AuthType.LOGIN_WITH_GOOGLE,
      undefined,
    ...
    expect(result).toContain(
      "[API Error: Quota exceeded for quota metric 'casualresearch 2.5 Pro Requests'",
    );
    expect(result).toContain(
      'You have reached your daily casualresearch-2.5-pro quota limit',
    );
    expect(result).toContain(
      'upgrade to a casualresearch Code Assist Standard or Enterprise plan',
    );
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Standard tier)', () => {
    ...
    expect(result).toContain(
      'We appreciate you for choosing casualresearch Code Assist and the casualresearch CLI',
    );
    expect(result).not.toContain(
      'upgrade to a casualresearch Code Assist Standard or Enterprise plan',
    );
  ...
  it('should handle different casualresearch 2.5 version strings in Pro quota exceeded errors', () => {
    const errorMessage25 =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'casualresearch 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    const errorMessagePreview =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'casualresearch 2.5-preview Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    ...
    const resultPreview = parseAndFormatApiError(
      ...
      AuthType.LOGIN_WITH_GOOGLE,
      undefined,
      'casualresearch-2.5-preview-pro',
      DEFAULT_casualresearch_FLASH_MODEL,
    );
    ...
    expect(resultPreview).toContain(
      'You have reached your daily casualresearch-2.5-preview-pro quota limit',
    );
    expect(result25).toContain(
      'upgrade to a casualresearch Code Assist Standard or Enterprise plan',
    );
  ...
  it('should not match non-Pro models with similar version strings', () => {
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'casualresearch 2.5 Flash Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'casualresearch 2.5-preview Flash Requests' and limit",
      ),
    ).toBe(false);
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'casualresearch 2.5 Ultra Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'casualresearch 2.5 Standard Requests' and limit",
      ),
    ).toBe(false);
