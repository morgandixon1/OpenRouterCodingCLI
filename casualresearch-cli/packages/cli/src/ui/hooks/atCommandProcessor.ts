...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const casualresearchIgnored =
      respectFileIgnore.respectcasualresearchIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectcasualresearchIgnore: true,
      });

    if (gitIgnored || casualresearchIgnored) {
      const reason =
        gitIgnored && casualresearchIgnored ? 'both' : gitIgnored ? 'git' : 'casualresearch';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and casualresearch'
          : reason === 'git'
            ? 'git-ignored'
            : 'casualresearch-ignored';
  ...
  const totalIgnored =
    ...
    ignoredByReason.casualresearch.length +
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason.casualresearch.length) {
      messages.push(`casualresearch-ignored: ${ignoredByReason.casualresearch.join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_casualresearch_ignore: respectFileIgnore.respectcasualresearchIgnore,
    },
