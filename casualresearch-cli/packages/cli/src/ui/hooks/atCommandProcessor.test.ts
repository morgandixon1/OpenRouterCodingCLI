...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectcasualresearchIgnore: () => true,
  ...
  describe('casualresearch-ignore filtering', () => {
    it('should skip casualresearch-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.casualresearchignore'),
        'build/output.js',
      );
      const casualresearchIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${casualresearchIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${casualresearchIgnoredFile} is casualresearch-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\ncasualresearch-ignored: ${casualresearchIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .casualresearchignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.casualresearchignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed casualresearch-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.casualresearchignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${casualresearchIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${validFile} @${casualresearchIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${validFile}:\n` },
