...
const getSavedChatTags = async (
  ...
  const casualresearchDir = context.services.config?.getProjectTempDir();
  if (!casualresearchDir) {
    return [];
  }
  try {
    const file_head = 'checkpoint-';
    const file_tail = '.json';
    const files = await fsPromises.readdir(casualresearchDir);
    const chatDetails: Array<{ name: string; mtime: Date }> = [];

    for (const file of files) {
      if (file.startsWith(file_head) && file.endsWith(file_tail)) {
        const filePath = path.join(casualresearchDir, file);
...
const saveCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<SlashCommandActionReturn | void> => {
    ...
    const chat = await config?.getcasualresearchClient()?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
...
const resumeCommand: SlashCommand = {
  ...
  action: async (context, args) => {
    ...
    const rolemap: { [key: string]: MessageType } = {
      ...
      model: MessageType.casualresearch,
    };
    ...
    for (const item of conversation) {
      ...
      if (i > 2 || !hasSystemPrompt) {
        uiHistory.push({
          type: (item.role && rolemap[item.role]) || MessageType.casualresearch,
          text,
