...
describe('setupGithubCommand', async () => {
  ...
  it('returns a tool action to download github workflows and handles paths', async () => {
    ...
    const expectedSubstrings = [
      ...
      `curl --fail --location --output "/github.com/fake/repo/root/.github/workflows/casualresearch-cli.yml" --show-error --silent`,
      `curl --fail --location --output "/github.com/fake/repo/root/.github/workflows/casualresearch-issue-automated-triage.yml" --show-error --silent`,
      `curl --fail --location --output "/github.com/fake/repo/root/.github/workflows/casualresearch-issue-scheduled-triage.yml" --show-error --silent`,
      `curl --fail --location --output "/github.com/fake/repo/root/.github/workflows/casualresearch-pr-review.yml" --show-error --silent`,
      `https://raw.githubusercontent.com/google-github-actions/run-casualresearch-cli/refs/tags/`,
    ];
