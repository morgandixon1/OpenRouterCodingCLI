...
import { loadServerHierarchicalMemory } from '@google/casualresearch-cli-core';
      ...
      action: async (context: CommandContext, args: string) => {
        ...
        try {
          if (config.shouldLoadMemoryFromIncludeDirectories()) {
            ...
            context.ui.setcasualresearchMdFileCount(fileCount);
          }
          addItem(
            {
              type: MessageType.INFO,
              text: `Successfully added casualresearch.md files from the following directories if there are:\n- ${added.join('\n- ')}`,
            },
            Date.now(),
          );
        } catch (error) {
        ...
        if (added.length > 0) {
          const casualresearch = config.getcasualresearchClient();
          if (casualresearch) {
            await casualresearch.addDirectoryContext();
          }
          addItem(
            {
              type: MessageType.INFO,
