...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const workflows = [
      'casualresearch-cli/casualresearch-cli.yml',
      'issue-triage/casualresearch-issue-automated-triage.yml',
      'issue-triage/casualresearch-issue-scheduled-triage.yml',
      'pr-review/casualresearch-pr-review.yml',
    ];
    ...
    for (const workflow of workflows) {
      ...
      const curlCommand = buildCurlCommand(
        `https://raw.githubusercontent.com/google-github-actions/run-casualresearch-cli/refs/tags/${releaseTag}/examples/workflows/${workflow}`,
        [`--output "${gitRepoRoot}/.github/workflows/${fileName}"`],
      );
      commands.push(curlCommand);
    }

    const readmeUrl = `https://github.com/google-github-actions/run-casualresearch-cli/blob/${releaseTag}/README.md#quick-start`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with casualresearch.',
        command,
      },
    };
  },
