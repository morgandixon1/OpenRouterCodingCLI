...
export const toolsCommand: SlashCommand = {
  name: 'tools',
  description: 'list available casualresearch CLI tools',
  ...
  action: async (context: CommandContext, args?: string): Promise<void> => {
    ...
    const casualresearchTools = tools.filter((tool) => !('serverName' in tool));

    let message = 'Available casualresearch CLI tools:\n\n';

    if (casualresearchTools.length > 0) {
      casualresearchTools.forEach((tool) => {
