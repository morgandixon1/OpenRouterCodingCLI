...
import { Config, GitService } from '@google/casualresearch-cli-core';
...
describe('restoreCommand', () => {
  ...
  let casualresearchTempDir: string;
  ...
  beforeEach(async () => {
    ...
    casualresearchTempDir = path.join(testRootDir, '.casualresearch');
    checkpointsDir = path.join(casualresearchTempDir, 'checkpoints');
    ...
    mockConfig = {
      ...
      getProjectTempDir: vi.fn().mockReturnValue(casualresearchTempDir),
      getcasualresearchClient: vi.fn().mockReturnValue({
  ...
  describe('action', () => {
    it('should return an error if temp dir is not found', async () => {
      ...
      expect(
        ...
        type: 'message',
        messageType: 'error',
        content: 'Could not determine the .casualresearch directory path.',
      });
    });
