...
import { usecasualresearchStream } from './hooks/usecasualresearchStream.js';
...
interface MockServerConfig {
  ...
  userAgent: string;
  userMemory: string;
  casualresearchMdFileCount: number;
  ...
  getcasualresearchMdFileCount: Mock<() => number>;
  setcasualresearchMdFileCount: Mock<(count: number) => void>;
  ...
  getAllcasualresearchMdFilenames: Mock<() => string[]>;
  getcasualresearchClient: Mock<() => casualresearchClient | undefined>;
  getUserTier: Mock<() => Promise<string | undefined>>;
  getIdeClient: Mock<() => { getCurrentIde: Mock<() => string | undefined> }>;
}

// Mock @google/casualresearch-cli-core and its Config class
  ...
  const ConfigClassMock = vi
    ...
    .mockImplementation((optionsPassedToConstructor) => {
      ...
      return {
        ...
        casualresearchMdFileCount: opts.casualresearchMdFileCount || 0,
        ...
        setUserMemory: vi.fn(),
        getcasualresearchMdFileCount: vi.fn(() => opts.casualresearchMdFileCount || 0),
        setcasualresearchMdFileCount: vi.fn(),
        ...
        getcasualresearchClient: vi.fn(() => ({
          getUserTier: vi.fn(),
        })),
        getCheckpointingEnabled: vi.fn(() => opts.checkpointing ?? true),
        getAllcasualresearchMdFilenames: vi.fn(() => ['casualresearch.md']),
...
vi.mock('./hooks/usecasualresearchStream', () => ({
  usecasualresearchStream: vi.fn(() => ({
    streamingState: 'Idle',
    submitQuery: vi.fn(),
    initError: null,
    pendingHistoryItems: [],
...
vi.mock('../config/config.js', async (importOriginal) => {
  ...
  return {
    ...
    ...actual,
    loadHierarchicalcasualresearchMemory: vi
      .fn()
...
const { isGitRepository: mockedIsGitRepository } = vi.mocked(
  await import('@google/casualresearch-cli-core'),
);
...
describe('App UI', () => {
  ...
  const createMockSettings = (
    ...
    const workspaceSettingsFile: SettingsFile = {
      path: '/workspace/.casualresearch/settings.json',
  ...
  describe('handleAutoUpdate', () => {
    ...
    afterEach(() => {
      delete process.env.casualresearch_CLI_DISABLE_AUTOUPDATER;
    });
    ...
    it('should not start the update process when running from git', async () => {
      ...
      const info: UpdateObject = {
        update: {
          name: '@google/casualresearch-cli',
          latest: '1.1.0',
          current: '1.0.0',
        },
        message: 'casualresearch CLI update available!',
      };
    ...
    it('should not auto-update if casualresearch_CLI_DISABLE_AUTOUPDATER is true', async () => {
      mockedIsGitRepository.mockResolvedValue(false);
      process.env.casualresearch_CLI_DISABLE_AUTOUPDATER = 'true';
      const info: UpdateObject = {
        update: {
          name: '@google/casualresearch-cli',
          latest: '1.1.0',
  ...
  it('should display active file and other context', async () => {
    ...
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(1);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue(['casualresearch.md']);
    ...
    await Promise.resolve();
    expect(lastFrame()).toContain(
      'Using: 1 open file (ctrl+e to view) | 1 casualresearch.md file',
    );
  });

  it('should display default "casualresearch.md" in footer when contextFileName is not set and count is 1', async () => {
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(1);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue(['casualresearch.md']);
    ...
    expect(lastFrame()).toContain('Using: 1 casualresearch.md file');
  });

  it('should display default "casualresearch.md" with plural when contextFileName is not set and count is > 1', async () => {
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(2);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue([
      'casualresearch.md',
      'casualresearch.md',
    ]);
    mockConfig.getDebugMode.mockReturnValue(false);
    ...
    currentUnmount = unmount;
    await Promise.resolve();
    expect(lastFrame()).toContain('Using: 2 casualresearch.md files');
  });
  ...
  it('should display custom contextFileName in footer when set and count is 1', async () => {
    ...
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(1);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue(['AGENTS.md']);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
  ...
  it('should display custom contextFileName with plural when set and count is > 1', async () => {
    ...
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(3);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue([
      'MY_NOTES.TXT',
      'MY_NOTES.TXT',
      'MY_NOTES.TXT',
  ...
  it('should not display context file message if count is 0, even if contextFileName is set', async () => {
    ...
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(0);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue([]);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
  ...
  it('should display casualresearch.md and MCP server count when both are present', async () => {
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(2);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue([
      'casualresearch.md',
      'casualresearch.md',
  ...
  it('should display only MCP server count when casualresearch.md count is 0', async () => {
    mockConfig.getcasualresearchMdFileCount.mockReturnValue(0);
    mockConfig.getAllcasualresearchMdFilenames.mockReturnValue([]);
    mockConfig.getMcpServers.mockReturnValue({
      server1: {} as MCPServerConfig,
  ...
  it('should render correctly with the prompt input box', () => {
    vi.mocked(usecasualresearchStream).mockReturnValue({
  ...
  describe('with initial prompt from --prompt-interactive', () => {
    it('should submit the initial prompt automatically', async () => {
      ...
      mockConfig.getcasualresearchClient.mockReturnValue({
        isInitialized: vi.fn(() => true),
        getUserTier: vi.fn(),
      } as unknown as casualresearchClient);
  ...
  describe('auth validation', () => {
    it('should call validateAuthMethod when useExternalAuth is false', async () => {
      ...
      mockSettings = createMockSettings({
        workspace: {
          selectedAuthType: 'USE_casualresearch' as AuthType,
      ...
      expect(validateAuthMethodSpy).toHaveBeenCalledWith('USE_casualresearch');
    });
