...
import { isGitRepository } from '@google/casualresearch-cli-core';

vi.mock('@google/casualresearch-cli-core', () => ({
...
describe('getInstallationInfo', () => {
  ...
  it('should detect running via npx', () => {
    const npxPath = `/Users/test/.npm/_npx/12345/bin/casualresearch`;
  ...
  it('should detect running via pnpx', () => {
    const pnpxPath = `/Users/test/.pnpm/_pnpx/12345/bin/casualresearch`;
  ...
  it('should detect running via bunx', () => {
    const bunxPath = `/Users/test/.bun/install/cache/12345/bin/casualresearch`;
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/usr/local/bin/casualresearch';
    process.argv[1] = cliPath;
    mockedRealPathSync.mockReturnValue(cliPath);
    mockedExecSync.mockReturnValue(Buffer.from('casualresearch-cli')); // Simulate successful command

    const info = getInstallationInfo(projectRoot, false);

    expect(mockedExecSync).toHaveBeenCalledWith(
      'brew list -1 | grep -q "^casualresearch-cli$"',
      { stdio: 'ignore' },
    );
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@google/casualresearch-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @google/casualresearch-cli@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@google/casualresearch-cli/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add @google/casualresearch-cli@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/bin/casualresearch`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @google/casualresearch-cli@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect local installation and identify yarn from lockfile', () => {
    const localPath = `${projectRoot}/node_modules/.bin/casualresearch`;
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    const globalPath = `/usr/local/bin/casualresearch`;
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g @google/casualresearch-cli@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
