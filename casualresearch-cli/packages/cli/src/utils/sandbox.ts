...
import { Config, SandboxConfig } from '@google/casualresearch-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'casualresearch-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'casualresearch-cli-sandbox';
const SANDBOX_PROXY_NAME = 'casualresearch-cli-sandbox-proxy';
...
function entrypoint(workdir: string): string[] {
  ...
  const cliCmd =
    process.env.NODE_ENV === 'development'
      ...
      : process.env.DEBUG
        ? `node --inspect-brk=0.0.0.0:${process.env.DEBUG_PORT || '9229'} $(which casualresearch)`
        : 'casualresearch';
...
export async function start_sandbox(
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      // start and set up proxy if casualresearch_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env.casualresearch_SANDBOX_PROXY_COMMAND;
    ...
    // determine full path for casualresearch-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under casualresearch-cli repo
    //
    // note this can only be done with binary linked from casualresearch-cli repo
    if (process.env.BUILD_SANDBOX) {
      if (!gcPath.includes('casualresearch-cli/packages/')) {
        console.error(
          'ERROR: cannot build sandbox using installed casualresearch binary; ' +
            'run `npm link ./packages/cli` under casualresearch-cli repo to switch to linked binary.',
        );
        process.exit(1);
      } else {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              casualresearch_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the casualresearch-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify casualresearch-cli-dev@google.com if the issue persists.';
      console.error(
    ...
    // casualresearch_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env.casualresearch_SANDBOX_PROXY_COMMAND;
    ...
    // copy casualresearch_API_KEY(s)
    if (process.env.casualresearch_API_KEY) {
      args.push('--env', `casualresearch_API_KEY=${process.env.casualresearch_API_KEY}`);
    }
    ...
    // copy casualresearch_MODEL
    if (process.env.casualresearch_MODEL) {
      args.push('--env', `casualresearch_MODEL=${process.env.casualresearch_MODEL}`);
    }
    ...
    for (const envVar of [
      'casualresearch_CLI_IDE_SERVER_PORT',
      'casualresearch_CLI_IDE_WORKSPACE_PATH',
      'TERM_PROGRAM',
    ]) {
    ...
    if (process.env.casualresearch_CLI_INTEGRATION_TEST === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
      ...
      // finally switch to that user to run the casualresearch process. This is
      // necessary on Linux to ensure the user exists within the
      // container's /etc/passwd file, which is required by os.userInfo().
      const username = 'casualresearch';
