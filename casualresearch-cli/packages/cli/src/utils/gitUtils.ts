...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-casualresearch-cli/releases/latest`;
    ...
    console.debug(`Failed to determine latest run-casualresearch-cli release:`, _error);
    throw new Error(
      `Unable to determine the latest run-casualresearch-cli release on GitHub.`,
    );
  }
};
