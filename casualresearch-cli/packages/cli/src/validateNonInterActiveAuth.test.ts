...
describe('validateNonInterActiveAuth', () => {
  let originalEnvcasualresearchApiKey: string | undefined;
  ...
  beforeEach(() => {
    originalEnvcasualresearchApiKey = process.env.casualresearch_API_KEY;
    originalEnvVertexAi = process.env.GOOGLE_GENAI_USE_VERTEXAI;
    originalEnvGcp = process.env.GOOGLE_GENAI_USE_GCA;
    delete process.env.casualresearch_API_KEY;
  ...
  afterEach(() => {
    if (originalEnvcasualresearchApiKey !== undefined) {
      process.env.casualresearch_API_KEY = originalEnvcasualresearchApiKey;
    } else {
      delete process.env.casualresearch_API_KEY;
    }
  ...
  it('uses USE_casualresearch if casualresearch_API_KEY is set', async () => {
    process.env.casualresearch_API_KEY = 'fake-key';
    const nonInteractiveConfig: NonInteractiveConfig = {
      refreshAuth: refreshAuthMock,
    };
    await validateNonInteractiveAuth(
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.USE_casualresearch);
  });
  ...
  it('uses USE_VERTEX_AI if both casualresearch_API_KEY and GOOGLE_GENAI_USE_VERTEXAI are set', async () => {
    process.env.casualresearch_API_KEY = 'fake-key';
    process.env.GOOGLE_GENAI_USE_VERTEXAI = 'true';
    process.env.GOOGLE_CLOUD_PROJECT = 'test-project';
    process.env.GOOGLE_CLOUD_LOCATION = 'us-central1';
  ...
  it('uses USE_casualresearch if GOOGLE_GENAI_USE_VERTEXAI is false, casualresearch_API_KEY is set, and project/location are available', async () => {
  ...
  it('uses configuredAuthType if provided', async () => {
    // Set required env var for USE_casualresearch
    process.env.casualresearch_API_KEY = 'fake-key';
    const nonInteractiveConfig: NonInteractiveConfig = {
      refreshAuth: refreshAuthMock,
    };
    await validateNonInteractiveAuth(
      AuthType.USE_casualresearch,
      undefined,
      nonInteractiveConfig,
    );
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.USE_casualresearch);
