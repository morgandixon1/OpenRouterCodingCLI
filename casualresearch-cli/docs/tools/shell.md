...
This document describes the `run_shell_command` tool for the casualresearch CLI.
...
## How to use `run_shell_command` with the casualresearch CLI
...
When `run_shell_command` executes a command, it sets the `casualresearch_CLI=1` environment variable in the subprocess's environment. This allows scripts or tools to detect if they are being run from within the casualresearch CLI.
