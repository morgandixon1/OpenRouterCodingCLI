# MCP servers with the casualresearch CLI

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the casualresearch CLI.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the casualresearch CLI through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the casualresearch model and your local environment or other services like APIs.

An MCP server enables the casualresearch CLI to:
...
- **Access resources:** Read data from specific resources (though the casualresearch CLI primarily focuses on tool execution).

With an MCP server, you can extend the casualresearch CLI's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The casualresearch CLI integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the casualresearch API
...
The casualresearch CLI supports three MCP transport types:
...
The casualresearch CLI uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.

### Configure the MCP server in settings.json

You can configure MCP servers at the global level in the `~/.casualresearch/settings.json` file or in your project's root directory, create or open the `.casualresearch/settings.json` file. Within the file, add the `mcpServers` configuration block.
...
The casualresearch CLI supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.casualresearch/mcp-oauth-tokens.json`
...
When the casualresearch CLI starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet casualresearch API requirements:
...
Tool parameter schemas undergo sanitization for casualresearch API compatibility:
...
When the casualresearch model decides to use an MCP tool, the following execution flow occurs:
...
Once discovered, MCP tools are available to the casualresearch model like built-in tools. The model will automatically:
...
- **Property stripping:** The system automatically removes certain schema properties (`$schema`, `additionalProperties`) for casualresearch API compatibility
- **Name sanitization:** Tool names are automatically sanitized to meet API requirements
- **Conflict resolution:** Tool name conflicts between servers are resolved through automatic prefixing

This comprehensive integration makes MCP servers a powerful way to extend the casualresearch CLI's capabilities while maintaining security, reliability, and ease of use.
...
When the casualresearch CLI receives this response, it will:
...
This enables you to build sophisticated tools that can provide rich, multi-modal context to the casualresearch model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within the casualresearch CLI. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="casualresearch CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "casualresearch CLI" reverent
```

When you run this command, the casualresearch CLI executes the `prompts/get` method on the MCP server with the provided arguments. The server is responsible for substituting the arguments into the prompt template and returning the final prompt text. The CLI then sends this prompt to the model for execution. This provides a convenient way to automate and share common workflows.

## Managing MCP Servers with `casualresearch mcp`

While you can always configure MCP servers by manually editing your `settings.json` file, the casualresearch CLI provides a convenient set of commands to manage your server configurations programmatically. These commands streamline the process of adding, listing, and removing MCP servers without needing to directly edit JSON files.

### Adding a Server (`casualresearch mcp add`)

The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.casualresearch/settings.json` or the project config `.casualresearch/settings.json` file.

**Command:**

```bash
casualresearch mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
casualresearch mcp add <name> <command> [args...]

# Example: Adding a local server
casualresearch mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
casualresearch mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
casualresearch mcp add --transport http <name> <url>

# Example: Adding an HTTP server
casualresearch mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
casualresearch mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
casualresearch mcp add --transport sse <name> <url>

# Example: Adding an SSE server
casualresearch mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
casualresearch mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"
```

### Listing Servers (`casualresearch mcp list`)
...
### Removing a Server (`casualresearch mcp remove`)
...
```bash
casualresearch mcp remove <name>
```

**Example:**

```bash
casualresearch mcp remove my-server
```
