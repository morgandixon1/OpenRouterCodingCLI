...
casualresearch CLI supports a variety of themes to customize its color scheme and appearance. You can change the theme to suit your preferences via the `/theme` command or `"theme":` configuration setting.

## Available Themes

casualresearch CLI comes with a selection of pre-defined themes, which you can list using the `/theme` command within casualresearch CLI:
...
1.  Enter `/theme` into casualresearch CLI.
...
Selected themes are saved in casualresearch CLI's [configuration](./configuration.md) so your preference is remembered across sessions.

---

## Custom Color Themes

casualresearch CLI allows you to create your own custom color themes by specifying them in your `settings.json` file. This gives you full control over the color palette used in the CLI.
...
- Select your custom theme using the `/theme` command in casualresearch CLI. Your custom theme will appear in the theme selection dialog.
