# Sandboxing in the casualresearch CLI

This document provides a guide to sandboxing in the casualresearch CLI, including prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the casualresearch CLI:
...
casualresearch -s -p "analyze the code structure"

# Use environment variable
export casualresearch_SANDBOX=true
casualresearch -p "run the test suite"
...
2. **Environment variable**: `casualresearch_SANDBOX=true|docker|podman|sandbox-exec`
...
```bash
DEBUG=1 casualresearch -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect casualresearch-cli due to automatic exclusion. Use `.casualresearch/.env` files for casualresearch-cli specific debug settings.

### Inspect sandbox

```bash
# Check environment
casualresearch -s -p "run shell command: env | grep SANDBOX"

# List mounts
casualresearch -s -p "run shell command: mount | grep workspace"
```
