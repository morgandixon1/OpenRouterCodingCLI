...
export class TestRig {
  constructor() {
    this.bundlePath = join(__dirname, '..', 'bundle/casualresearch.js');
    this.testDir = null;
  }
  ...
  getDefaultTimeout() {
    ...
    if (env.casualresearch_SANDBOX) return 30000; // 30s in containers
  ...
  setup(testName, options = {}) {
    ...
    const casualresearchDir = join(this.testDir, '.casualresearch');
    mkdirSync(casualresearchDir, { recursive: true });
    // In sandbox mode, use an absolute path for telemetry inside the container
    // The container mounts the test directory at the same path as the host
    const telemetryPath =
      env.casualresearch_SANDBOX && env.casualresearch_SANDBOX !== 'false'
    ...
    const settings = {
      ...
      sandbox: env.casualresearch_SANDBOX !== 'false' ? env.casualresearch_SANDBOX : false,
      ...options.settings, // Allow tests to override/add settings
    };
    writeFileSync(
      join(casualresearchDir, 'settings.json'),
    ...
    const promise = new Promise((resolve, reject) => {
      child.on('close', (code) => {
        if (code === 0) {
          ...
          let result = stdout;
          if (env.casualresearch_SANDBOX === 'podman') {
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'casualresearch_cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const jsonStr of jsonObjects) {
      try {
        ...
        if (
          logData.attributes &&
          logData.attributes['event.name'] === 'casualresearch_cli.tool_call'
        ) {
